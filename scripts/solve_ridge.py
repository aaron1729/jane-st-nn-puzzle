...
def ridge_mse(h, y, alpha=1.0):
    """Fit ridge regression from h to y, return training MSE."""
    # Add intercept column
    H = np.hstack([h, np.ones((h.shape[0], 1))])  # [N, 49]
    # Ridge: (H'H + alpha*I)^{-1} H'y  (don't penalize intercept)
    HtH = H.T @ H
...
# Greedy search with ridge scoring
h = X.copy()
remaining_expand = set(expand_ids)
remaining_contract = set(contract_ids)
chosen_order = []

null_mse = ridge_mse(h, pred)
print(f"Initial (ridge on raw input) MSE: {null_mse:.6f}")
print()
...
for step in range(N_STEPS):
    ...
    print(f"  Top 5:  {[(f'{s:.6f}', e, c) for s, e, c in all_scores[:5]]}")
    print(f"  Worst 5: {[(f'{s:.6f}', e, c) for s, e, c in all_scores[-5:]]}")
    if len(all_scores) > 1:
...
# Build full permutation and validate with actual LastLayer
...
print(f"\nFinal MSE (actual LastLayer) vs pred: {final_mse:.8f}")
